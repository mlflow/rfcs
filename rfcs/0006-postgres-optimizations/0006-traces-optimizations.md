...
Rollup rows use a database-generated primary key and store `workspace`, `experiment_id`,
`rollup_day`, `metric_name`, and `grouping_set` as ordinary columns. `grouping_set` identifies
the dimensions used to produce the row. Its values are `global` and `status` for trace rollups;
`global`, `model`, `provider`, and `model_provider` for span-cost rollups; and `global` for
assessment rollups. Null dimension values are not included in grouped sets. `global` means the
metric is aggregated across all rows for the workspace, experiment, and day without grouping by
any optional dimension. Global rows are identified by `grouping_set`, not by null dimension
columns.
...
`sql_trace_rollup_rebuild_queue` is keyed by `(workspace, experiment_id, rollup_day, rollup_family)`
and acts as a durable rebuild queue: row presence means that the family and day require rebuilding.
A reader uses a family's rollups for a day only when matching rollup rows exist and no rebuild entry
exists.
...
CREATE INDEX idx_span_cost_rollups_lookup
  ...
  (workspace, experiment_id, rollup_day, metric_name, grouping_set, model_name, model_provider);
...
2. Deploy the new version and run its Alembic migration. The migration adds the columns, performs
   ...
   contains only model and provider keys.
...
of the indexes above. Use `Float(53)` or `DOUBLE PRECISION` for denormalized numeric columns.
Large legacy-row cleanup may require normal autovacuum follow-up or `VACUUM (ANALYZE)`.
